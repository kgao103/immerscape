...
var getIntersectingTile = function (screenPosition) {
  if (
    screenPosition[0] >= gridOrigin[0] &&
    screenPosition[0] <= gridOrigin[0] + BOARDSIZE &&
    screenPosition[1] >= gridOrigin[1] &&
    screenPosition[1] <= gridOrigin[1] + BOARDSIZE
  ) {
...
var getIntersectingShipAndOffset = function (screenPosition) {
  ...
  if (intersectShip) {
    ...
    var offset = [
      screenPosition[0] - shipPosition[0],
...
var highlightTile = function (position, color) {
  tiles[position.row * NUMTILES + position.col].setProperties({
...
// blinkTiles()
//    Causes all of the tiles to blink
var blinkAllTiles = function () {
  var angle = 0;
  tileModifiers.forEach(function (modifier) {
...
var blinkTile = function (position) {
  var angle = 0;
  tileModifiers[position.row * NUMTILES + position.col].opacityFrom(
    function () {
      angle += 0.1;
      return Math.cos(angle);
    }
